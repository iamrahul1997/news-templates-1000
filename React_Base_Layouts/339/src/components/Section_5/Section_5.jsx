...
export default function Section4() {
  const { data, isLoading, isError, error } = useQuery({
    ...
    queryFn: () => getPosts(4), // replace 4 with your API section number
  });
  ...
  if (postsArray.length < 39) return null;

  const topArticle = postsArray[34]; // Full-width top article
  const bottomArticles = postsArray.slice(35, 39); // 4 smaller articles
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-6 lg:px-8">
        ...
        {topArticle && (
          <Link
            ...
            className="relative w-full h-[400px] md:h-[500px] rounded-3xl overflow-hidden mb-8 group block">
            <img
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
          {bottomArticles.map((article) => (
            <Link
              ...
              className="relative h-[220px] md:h-[250px] rounded-2xl overflow-hidden group block">
              <img
